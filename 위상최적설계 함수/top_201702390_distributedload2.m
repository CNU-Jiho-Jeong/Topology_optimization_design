...
function top_201702390_distributedload2(nelx,nely,volfrac,penal,rmin);
% top_201702390_distributedload2 함수 정의
...
% 외부에서 작용하는 힘 정의(대각선 분포하중)
F(2*(nely+1):2*(nely+1):2*(nelx+1)*(nely+1),1) = 1*sin(45);
F(2*(nely+1)-1:2*(nely+1):2*(nelx+1)*(nely+1)-1,1) = -1*cos(45);



% 고정된 자유도(첫번째 자유도 + 맨 윗 node들의 y축 방향 자유도 + 맨 윗 node들 중 마지막 node의 x축 방향 자유도)
